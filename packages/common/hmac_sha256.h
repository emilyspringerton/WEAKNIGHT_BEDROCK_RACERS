...
// dependencies. Ported verbatim (2026-08-28, "build login from the beginning
// take it from GFD") from shankpit-460's own real, RFC-4231-verified
// implementation (GoblinFoxDragon/apps2/battlegrounds_gui/packages/common/
// hmac_sha256.h) -- same real reasoning applies here: this repo has no
...
// authenticates via POST /api/v1/auth/email/login; the racer game server
// (apps/server) verifies the HMAC locally with a shared secret (see
// racer_protocol.h's own RC_TICKET_* constants) -- no asymmetric crypto, no
// blocking network call per connect.
#ifndef RACER_HMAC_SHA256_H
#define RACER_HMAC_SHA256_H
